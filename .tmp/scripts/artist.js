...
(function (createjs) {
  ...
  var getAABB = function getAABB(stroke) {
  ...
  stage.on('stagemousemove', function (event) {
    if (drawMode) {
      if (!shape) {
        ...
        start = origin;
        stroke.push(start);
        stage.addChild(shape).graphics.beginStroke('red').update();
      }
      next = {
  ...
  stage.on('stagemouseup', function () {
    ...
    var distance = Math.sqrt(Math.pow(last.x - origin.x, 2) + Math.pow(last.y - origin.y, 2));
