...
#define itemWithString itemFromString
    ...
    CCMenuItem* reloadItem = [CCMenuItemFont itemWithString:@"Reload" target:self selector:@selector(loadWorld:)];
...
- (b2Fixture*)getTouchedFixture:(UITouch*)touch
{
    CGPoint screenPos = [touch locationInView:[touch view]];
    b2Vec2 worldPos = [self screenToWorld:screenPos];
