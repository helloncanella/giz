...
@interface PlanetCuteRUBELayer : RUBELayer
{
    @public b2Body* m_playerBody;                       // duh...
    
    @public b2Fixture* m_footSensorFixture;             // a small sensor fixture attached to the bottom of the player body to detect when it's standing on something
    @public int m_numFootContacts;                      // the current number of other fixtures touching the foot sensor. If this is > 0 the character is standing on something
    ...
    NSMutableArray* m_allPickups;                       // a array containing every pickup in the scene. This is used to loop through every tick and make them wobble around.
    NSMutableSet* m_pickupsToProcess;                   // The contact listener will put some pickups in this set when the player touches them. After the Step has finished, the
    ...
    UITouch* m_leftTouch;                               // used to keep track of when the user touched the left side of the screen (if this is nil, the left side is not touched)
    UITouch* m_rightTouch;                              // used to keep track of when the user touched the right side of the screen (if this is nil, the right side is not touched)
